...
<?php
	...
	if (isset($_POST['btnApproved']))
	{
		...
		{
			header("Location:../view/EmployeeOnLeave.php");
		}
		else
		{
			echo "Sorry";
	...
	if(isset($_POST['btnFinalSubmit']))
	{
        ...
        $leaveToDate = $objForLeaveDaysRemain->forLeaveToDate();
//			var_dump($leaveToDate);
        $extractYear = date("y", strtotime($leaveToDate));
        $currentYear = date("Y");

		if (mysqli_fetch_array($result) == null || $extractYear < $currentYear)
		{
			$objForLeaveDaysRemain = new LeaveApplication();
			$leaveTotal  = $objForLeaveDaysRemain->forLeaveDays($livLeaveId);

            if ($livTotalLeaveDays > $leaveTotal){

                $_SESSION['msgForApplication'] = "You don't have enough leave days";
                header("Location:../view/LeaveApplication.php");
            }
            else{
                $rowTotal = mysqli_fetch_array($leaveTotal);
...
//			var_dump($leaveToDate);
			$extractYear = date("y", strtotime($leaveToDate));
			$currentYear = date("Y");
			if ($extractYear == $currentYear){

                if ($livTotalLeaveDays > $result){
                    $_SESSION['msgForApplication'] = "You don't have enough leave days";
                    header("Location:../view/LeaveApplication.php");
                }
                else{
